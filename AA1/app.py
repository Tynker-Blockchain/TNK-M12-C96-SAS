...
def home():
    ...
    if(nodeId not in allNodes):
        ...
        # Loop through each nodeId in allNodes
        for nodeId in allNodes:
            # Check if length of blockchain of node is greater then 0 and length of peerBlocks is 0
            if (len(allNodes[nodeId].blockchain.chain)>0)and len(allNodes[nodeId].blockchain.peerBlocks) == 0:
                # Copy chain from current iteration node and save it in currentNode's blockchain 
                currentNode.blockchain.chain = allNodes[nodeId].blockchain.chain
